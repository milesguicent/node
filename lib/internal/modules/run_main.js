...
function shouldUseESMLoader(mainPath) {
  ...
  const entryPointExtention = getApplicableMainEntryPointExtention(mainPath)
  // Determine the module format of the entry point.
  if (mainPath && entryPointExtention === '.mjs') { return true; }
  if (mainPath && entryPointExtention === '.wasm') { return true; }
  if (!mainPath || entryPointExtention === '.cjs') { return false; }

  if (getOptionValue('--strip-types')) {
    if (!mainPath || entryPointExtention === '.cts') { return false; }
    // This will likely change in the future to start with commonjs loader by default
    if (mainPath && entryPointExtention === '.mts') { return true; }
  }

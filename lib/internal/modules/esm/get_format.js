...
function getFileProtocolModuleFormat(url, context = { __proto__: null }, ignoreErrors) {
  const { source, parentURL } = context;

  // when parentURL is undefined, that means we are loading the entrypoint
  const ext = !parentURL ? getApplicableMainEntryPointExtention(url.pathname) : extname(url);

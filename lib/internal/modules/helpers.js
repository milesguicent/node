...
/**
 * Extract the extention of the main file, or use the `--experimental-ext` as fallback
 * @param {string | null | undefined} the path to the main file
 * @returns {string | null} The extention, with the preceding dot
 */
function getApplicableMainEntryPointExtention(mainPath) {
  if (!mainPath) return null

  const fileExtention = path.extname(mainPath);
  if (fileExtention !== '') return fileExtention;

  /**
   * @type {string} extention A fallback extention to use when the entrypoint has no extention
   */
  const cliExtention = getOptionValue('--experimental-ext');
  if (cliExtention) {
    return cliExtention[0] === '.' ? cliExtention : `.${cliExtention}`;
  }
...
module.exports = {
  ...
  enableCompileCache,
  flushCompileCache,
  getApplicableMainEntryPointExtention,
  getBuiltinModule,
  getCjsConditions,
